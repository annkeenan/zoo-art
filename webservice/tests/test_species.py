...
class TestExhibit(unittest.TestCase):
	...
	def test_get_species(self):
		...
		resp = json.loads(r.content.decode())
		self.assertEqual(resp['result'], 'success')
		self.assertEqual(resp['ara glaucogularis'], {'common name': ['Blue-Throated Macaw','Caninde Macaw', 'Wagler\'s Macaw'],
				'genus': 'ara',
				'family': 'psittacidae',
				'region': ['neotropical'],
	...
	def test_put_species(self):
		...
		r = requests.put(self.SPECIES_URL + 'ara glaucogularis', data=json.dumps(reqBody))
		self.assertTrue(self.is_json(r.content.decode()))
		resp = json.loads(r.content.decode())
		self.assertEqual(resp['result'], 'success')
	...
	def test_post_species(self):
		...
		resp = json.loads(r.content.decode())
		self.assertEqual(resp['result'], 'success')
		self.assertEqual(resp['canis dirus'], {'common name': ['Dire wolf'],
            'genus': 'canis',
            'family': 'canidae',
            'region': ['holarctic'],
            'habitat': ['tundra'],
